...
function oc_category_to_store(&$result, $connect_bd)
{
    ...
    $query_new = "INSERT INTO `oc_category_to_store` (`category_id`, `store_id`) VALUES ";
    foreach ($data as $k) {
        foreach ($k as $kq => $v) {
            if (($v["id"]) !== null) {
                $query_new .= "(" . ($v["id"]) . ", " . 0 . ")";
    ...
    $query_new .= " ON DUPLICATE KEY UPDATE `category_id` = VALUES(`category_id`), `store_id` = VALUES(`store_id`);";
    $res_query_new = mysqli_query($connect_bd, $query_new);
    if (!$res_query_new) {
        echo "Запрос: " . $query_new . "<br/>" . "Сообщение ошибки:" . "<br/>" . mysqli_error($connect_bd);
    } else {
        echo "Запрос к --OC_CATEGORY_STORE-- успешно сформирован и отправлен. Ошибоки отсутствуют;" . "<br/>";
...
while ($result) {
    oc_category_to_store($result, $connect_bd);
}
