...
$query1 = mysqli_query($connect_bd, "DELETE FROM `oc_product_attribute`");
function oc_product_attribute(&$result, $connect_bd)
{
    ...
    $query_new = "INSERT INTO `oc_product_attribute` (`product_id`, `attribute_id`, `text`, `language_id`) VALUES";
    foreach ($data as $k) {
        foreach ($k as $kq => $v) {
            foreach (($v["facet_list"]) as $atribute_text) {
                $Atribute_text_value = ($atribute_text["value"]);
                $sku = ($v["sku"]);
                ...
                foreach ($arrayAtribute_text_id as $Atribute_text_id) {
                    $query_new .= " ("
                    . $sku . ", "
                    . ($Atribute_text_id["attribute_id"]) . ", '"
                    . mysqli_real_escape_string($connect_bd, $Atribute_text_value) . "', "
                        . 1 . "),";
                }
            }
        }
    ...
    $query_new1 .= " ON DUPLICATE KEY UPDATE
