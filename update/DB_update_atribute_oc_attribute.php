...
ini_set("memory_limit", "512M");
	...
	function parser(&$result, $connect_bd){
			if($data = json_decode($result, true)){  // Декодирование получаемых данных и преобразование в массив.
					...
					foreach ($k as $kq => $v){
						foreach(($v["facet_list"]) as $atribute_text){
							...
							foreach($arrayAtribute_text_id as $Atribute_text_id){ 
								...
								$query_new .= "(" . $sku . ", " . ($Atribute_text_id["attribute_id"]) . ", " . $att . ")" ;
		 						
		 						if ($Atribute_text_id+1 < count($arrayAtribute_text_id) { 
								$query_new .= ", ";
								}
							}
							
							if ($atribute_text+1 < count($v["facet_list"])) { 
								$query_new .= ", ";
							}
						}		
					}
				...
				$query_new .= " ON DUPLICATE KEY UPDATE `product_id` = VALUES(`product_id`), `attribute_id` = VALUES(`attribute_id`), `text` = VALUES(`text`);";
